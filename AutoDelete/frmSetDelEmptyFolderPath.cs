...
{
    public partial class frmSetDelEmptyFolderPath : Form
    {
        AutoDeleteHiddenForm frm;
        List<EmptyPath> lst_EmptyPath;
        public frmSetDelEmptyFolderPath()
        {
            InitializeComponent();
        }
        ...
        public void SetMainForm(AutoDeleteHiddenForm frm)
        {
            ...
            foreach (EmptyPath p in frm.lst_ClearPath)
            {
                lst_EmptyPath.Add(p);
            }
        }
        ...
        private void frmSetDelEmptyFolderPath_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            frm.KillSetDelEmptyFolderPathForm(strValue);
        }
        ...
        private void frmSetDelEmptyFolderPath_Load(object sender, EventArgs e)
        {
            foreach(EmptyPath path in frm.lst_ClearPath)
            {
        ...
        {
            foreach (ListViewItem item in lstv_Path.Items)
            {
                if (item.Checked == true)
                    CheckAndDeleteEmptyFolderStandalone(item.Text);
            }
            MessageBox.Show("完成");
        }
        ...
        private bool CheckAndDeleteEmptyFolderStandalone(String strPath) // testing 
        {
            ...
            try
            {
                isEmpty = !Directory.EnumerateDirectories(strPath).Any();
                if (isEmpty == false)
                {
